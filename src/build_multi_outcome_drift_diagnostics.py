...
def save_retention_heatmap(long: pd.DataFrame) -> None:
    ...
    plt.title("Multi-outcome retention from outcome-observed rows to analytical samples")
...
def save_primary_waterfall(long: pd.DataFrame) -> None:
    ...
    ax.set_title("Primary outcome attrition from monitoring target to analytical samples")
