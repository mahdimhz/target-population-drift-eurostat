...
\centering
\scriptsize
\caption{Final classification of Eurostat unmet-care findings. Labels are generated from the drift-stability summary and deterministic rules; they summarize whether a poverty/social-exclusion coefficient can be interpreted for each outcome target.}
...
Table~\ref{tab:final_classification_eurostat_findings} summarizes the empirical conclusion from the rule-based drift-stability framework. Many population-denominator poverty/social-exclusion coefficients are missingness-dependent because complete-case or IPW estimates and MAR full-target estimates disagree enough to change the coefficient interpretation. Need-denominator outcomes are non-identifiable as coefficient targets in this public aggregate setup because too few estimand families are feasible. Target-dependent labels are reserved for outcomes where the generated matrix shows material movement across weighting or target restrictions under the tolerance rule. Stable would mean cautious aggregate interpretation is supportable under that rule, not that an individual-level mechanism has been identified.

\section{What the Simulation Adds}

The semi-synthetic simulation strengthens the audit logic because it creates a reference estimand before deleting covariates. It does not claim to know a population coefficient outside the constructed reference panel. It asks a narrower question: when a complete artificial panel is available and Eurostat-style missingness is imposed, how often do complete-case FE, population-weighted FE, IPW, PMM MI, and MNAR-shift MI recover the reference estimand?
