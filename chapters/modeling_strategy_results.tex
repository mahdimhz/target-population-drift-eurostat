...
Chapter~\ref{ch:discussion_conclusion} applies the deterministic labels once, after the empirical and simulation evidence has been presented. This avoids repeating the same outcome classifications in both the results and conclusion chapters.
...
\begin{minipage}{0.96\textwidth}
\scriptsize \textit{Source:} Author's semi-synthetic simulation based on the complete-case Eurostat primary panel. The reference estimand is defined inside the simulation; it is not a claim about a true European causal coefficient.
\end{minipage}
\end{table}
...
\begin{minipage}{0.94\textwidth}
\scriptsize \textit{Source:} Author's semi-synthetic missingness simulation. The plot links sample drift to coefficient error as a diagnostic relationship, not as a formal bias theorem.
\end{minipage}
\end{figure}
