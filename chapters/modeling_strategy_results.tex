...
Table~\ref{tab:outcome_estimand_coefficient_matrix} replaces separate robustness rows with one outcome-by-estimand audit. For each feasible outcome, it follows the poverty/social-exclusion coefficient across unweighted complete-case FE, population-weighted FE, balanced-outcome FE, IPW FE, and MAR MI full-target FE; rows below the support rule are reported as infeasible rather than forced.

The matrix changes the interpretation of the evidence. Long population-denominator indicators support several observed-data estimands, while need-denominator indicators are useful for monitoring but often too short for stable complete-case regression targets. Figure~\ref{fig:outcome_estimand_coefficient_stability} shows the same point graphically: observed complete-case estimates often remain positive, whereas MAR MI full-target estimates can attenuate or change sign.

Table~\ref{tab:drift_stability_summary} translates the matrix into the framework metrics from Chapter~\ref{ch:framework}. The standardized coefficient \(\theta\) puts coefficients on an IQR scale; \(\theta\) IQR summarizes dispersion; directional agreement records sign agreement; and CSI applies the declared \(\delta=0.10\) tolerance rule. The final labels are deterministic diagnostics, not substantive effect claims.
