...
The simulation also calculates the target-population drift components defined in Chapter~\ref{ch:framework} for every mechanism--estimator draw. Complete-case, weighted, and IPW estimators are evaluated on their retained analytical rows. PMM and MNAR-shift MI estimators are evaluated as full-row model-based targets; their row drift is therefore zero by construction, but their coefficient errors still diagnose imputation-model dependence. The complete generated validation table reports mean TDI, mean bias, median absolute coefficient error, sign-reversal rate, interval coverage, wrong-conclusion rate, and failed replicates for every estimator.
...
Table~\ref{tab:simulation_main_summary} shows that the direction of failure depends on the missingness mechanism. Complete-case FE is relatively stable under country-group, covariate-block, MAR-style, and Eurostat-realistic deletion in this reference panel, but it shows sign reversals under MCAR deletion because the retained rows become small and noisy. PMM MI and MNAR-shift MI are not uniformly better: under early-year and country-group deletion, they can move far from the reference estimand. This is the simulation analogue of the thesis's main empirical caution: missing-data methods change assumptions and target populations; they do not automatically repair public-data non-coverage.
...
\begin{minipage}{0.94\textwidth}
\scriptsize \textit{Source:} Author's semi-synthetic missingness simulation. Bias is measured relative to the simulation reference estimand after imposing each missingness mechanism.
\end{minipage}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.96\textwidth]{simulation_tdi_vs_error}
\caption{Target-Population Drift Index and absolute coefficient error in the semi-synthetic simulation. TDI summarizes row, country, year, weighting, and standardized balance movement from the reference panel; the figure evaluates whether larger diagnostic drift coincides with larger coefficient error by estimator, not whether TDI proves bias.}
\label{fig:simulation_tdi_vs_error}
\vspace{0.25em}

\begin{minipage}{0.94\textwidth}
\scriptsize \textit{Source:} Author's semi-synthetic missingness simulation. The plot links target drift to coefficient error as a diagnostic relationship, not as a formal bias theorem.
\end{minipage}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.94\textwidth]{simulation_wrong_conclusion_by_tdi}
\caption{Wrong-conclusion probability by TDI bin in the semi-synthetic simulation. A wrong conclusion is recorded when the estimated sign differs from the reference estimand or when an interval excludes zero in the wrong direction; the figure evaluates whether higher diagnostic drift is associated with more frequent wrong conclusions across estimators.}
\label{fig:simulation_wrong_conclusion_by_tdi}
\vspace{0.25em}

\begin{minipage}{0.94\textwidth}
\scriptsize \textit{Source:} Author's semi-synthetic missingness simulation. Bins summarize the generated simulation draws and are used only as an audit diagnostic.
\end{minipage}
\end{figure}

Figures~\ref{fig:simulation_bias_by_mechanism}, \ref{fig:simulation_tdi_vs_error}, and \ref{fig:simulation_wrong_conclusion_by_tdi} connect the simulation to the audit protocol. Larger TDI values are not a formal theorem of bias, but they make visible when the retained analytical sample has moved away from the reference panel. The simulation contribution is therefore methodological-applied: it shows why public aggregate-panel studies should report target definition, row loss, selection distortion, estimator sensitivity, and non-identifiability before interpreting substantive coefficients.
