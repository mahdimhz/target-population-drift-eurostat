...
\]
with uncertainty pooled using Rubin's rules. This notation is a reminder that MI changes the target and adds model dependence; it does not reveal the true coefficient.
...
Missingness is a substantive part of the empirical design. Some Eurostat indicators have shorter time coverage than \texttt{hlth\_silc\_08}, and some are unavailable for specific countries. The grouped heatmaps below show the country-year pattern of missingness in the merged panel. They are used as data-quality and sample-construction diagnostics, not as substantive results by themselves.

Figures~\ref{fig:missingness_heatmap_socioeconomic} and \ref{fig:missingness_heatmap_financing} show two of the grouped missingness heatmaps used to diagnose coverage. Additional grouped heatmaps are included in the reproducibility materials generated with the thesis.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.96\textwidth]{missingness_heatmap_socioeconomic}
\caption{Socioeconomic covariate coverage across country-year cells in the merged national panel. The figure covers the 2008--2025 outcome-observed construction window; white or light cells indicate missing covariate coverage, and country coverage varies by variable and year.}
...
\caption{Health-financing covariate coverage across country-year cells in the merged national panel. The figure covers the 2008--2025 outcome-observed construction window; missing cells show why complete-case regression uses fewer rows than the outcome panel.}
